...
% Make fixed-with autoref spacing by (1) defining a fixed-width space (a kern, not a skip) and (2) redefining \nobreakspace, which is used as spacing command inside hyperref, to be that fixed-width space in each call to \autoref.
...
\def\equationautorefname{Eq.\fixedeqspace}
